...
\cite{Kreidberg2015}, and \cite{Parviainen2015}, these approaches tend to be slower, they have precisions
...
$b < b_0$, where $b_0$ is a cutoff value, which avoids division by $b$:
\begin{eqnarray}
...
\end{equation}
Since this recursion relation only involves every other term, we only need the two lowest terms,
which are given by:
\begin{eqnarray}
...
With the description of the integration complete, we next discuss
...
the transit with high fidelity \citep{Kipping2010}.  For constructing
...
\begin{eqnarray}
\overline{\mathcal{F}} &=& \frac{1}{\Delta t} \int_{t-\tfrac{1}{2}\Delta t}^{t+\tfrac{1}{2}\Delta t} \mathcal{F}(t) dt,\\
\frac{\partial \overline{\mathcal{F}}}{\partial r} &=& \frac{1}{\Delta t} \int_{t-\tfrac{1}{2}\Delta t}^{t+\tfrac{1}{2}\Delta t} \frac{\partial \mathcal{F}(t)}{\partial r} dt,\\
%\frac{\partial \overline{\mathcal{F}}}{\partial b}&=& \frac{1}{\Delta t} \int_{t-\tfrac{1}{2}\Delta t}^{t+\tfrac{1}{2}\Delta t} \frac{\partial \mathcal{F}(t)}{\partial b} dt,\\
\frac{\partial \overline{\mathcal{F}}}{\partial d_i}&=& \frac{1}{\Delta t} \int_{t-\tfrac{1}{2}\Delta t}^{t+\tfrac{1}{2}\Delta t} \frac{\partial \mathcal{F}(t)}{\partial d_i} dt,\\
\frac{\partial \overline{\mathcal{F}}}{\partial \bvec{x}} &=& \frac{1}{\Delta t}
\int_{t-\tfrac{1}{2}\Delta t}^{t+\tfrac{1}{2}\Delta t} \frac{\partial \mathcal{F}}{\partial b}\frac{\partial b(t)}{\partial \bvec{x}} dt,
\end{eqnarray}
...
As an example, we choose the approximate model $b(t) = (b_0^2 + v^2(t-t_0)^2)^{1/2}$,
which ignores acceleration and curvature during a transit,
and compute the time-averaged flux and derivatives for a length of integration time.
The integration is carried out with an adaptive refinement of the interval by
...
impact parameter model.  The time-integration smooths both the features of
the light curve, as well as the features of the derivative curves.
...
to $u_j$ can be expensive to apply, we can carrying out the gradient of the 
likelihood fucntion with respect to $d_i$, and then apply the Jacobian 
...
DobbsDixon2012};  limb-
darkening is treated as azimuthally-symmetric \citep[but see][]{Barnes2009a}; refraction
and any relativistic effects are ignored \citep[but see][]{Sidis2010}; the edges of both
...
The parameterization of limb-darkening can impact the precision of the computation
...
and can only explore a finite set of parameters (effective temperature,
metallicity, surface gravity, and magnetic field strength).
In practice, then, it may be most robust to simply let the limb-darkening parameters 
be free parameters, let the limb-darkening model be as flexible as possible, 
