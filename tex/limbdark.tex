...
at the limb.  In terms of $r$, the normalized radius projected on the sky,
$\upmu = \cos{\theta} =\sqrt{1-r^2}$, where $0\le r \le 1$.
...
limb-darkening model with respect to $b$, $r$, and the two quadratic limb-
darkening coefficients.  In this work, we give modified expressions which are 
...
and \cite{Kreidberg2015}, these approaces tend to be slower, they have precisions
...
in length to the spine of the kite.  The square root in this formula (equation 
...
in the arccosine case), which can improve numerical stability for small values of $b$ or $r$.
...
Note that since $\upmu = \sqrt{1-r^2}$, this problem is equivalent to 
...
leads to analytic expressions for the derivatives, which we find can
be faster (CHECK THIS) and more accurate to evaluate than with automatic
differentiation.
...
looping over the binomial expansion of each of the $u_n$ terms (equation \ref{eq:polynomialld}), for
...
as $n^{1/3}$.  As with the number of light-curve points, we have taken
...
much as a fex $\times 10^{-8}$ for the flux and the derivatives with
...
The imprecision of the model can be absorbed into noise models that account
for outliers or account for correlations in the noise.  Since we are limited
in the knowledge of the properties of any given star, the discrepancies of
an azimuthally-symmetric limb-darkened model can be treated as a source
of noise.
...
and accurate than prior models, especially in special limits such as
the points of contact, and the coincidence of the edge of the occultor
with the center of the source.  The model also compares favorably in
...
\begin{enumerate}
\item In equation 7, $\lambda_4$ should read $E(2p)$ and $K(2p)$,
{\it not} $E(2k)$ and $K(2k)$.  This mistake is not present in the code.

\item In equation 7, $\lambda_3$ should read $E(1/2p)$ and $K(1/2p)$,
{\it not} $E(1/2k)$ and $K(1/2k)$.  This mistake is also not present in the code.

\item In equation 7, $\lambda_5$ should read:
$\lambda_5 = \frac{2}{3\pi}\cos^{-1}(1-2p)-\frac{4}{9\pi}(3+2p-8p^2)
\sqrt{p(1-p)} - \frac{2}{3}\Theta(p-1/2)$.

(The last term cancels the $\Theta(p-z)$ term in the expression for $F$
just before equation 7).  This mistake is also not present in the code.
...
Pal 2008).  This case is rarely encountered as it is a set
of measure zero.

\item There is a $\pi$ missing in the denominator of the second term
on the right hand side of equation (8).  This mistake is not
present in the code.
\end{enumerate}
