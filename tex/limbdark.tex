...
% Redefine this to \bf to show changes
\newcommand{\edited}{}
...
\begin{dcases}
0 & \qquad 1+r \le b,\\
r^2 \kappa_0 + \kappa_1 -\sqrt{\frac{4b^2-(1+b^2-r^2)^2}{4}} & \qquad \vert 1-r\vert < b < 1+r,\\
...
%\frac{\partial \overline{F}}{\partial b}&=& \frac{1}{\Delta t} \int_{t-\tfrac{1}{2}\Delta t}^{t+\tfrac{1}{2}\Delta t} \frac{\partial F(t^\prime)}{\partial b} dt^\prime,\cr
...
\begin{figure}
    ...
    $t_0 = 0$, {\edited $v=1$}, $b_0=0.5$, and $u_1=u_2=0.3$. \jlcodelink{integrate_transit_gradient}
...
\begin{figure}
    ...
    time per exposure relative to a single evaluation per exposure. \jlcodelink{profile_integrate_transit}
...
%For large values of $N$, the computing time for
...
Despite the large values of $\mathfrak{g}_n$, the light curve computed has a
much smaller value with a depth of $\approx 1$\% for a planet with a radius ratio 
of $r=0.1$ due to a fine-tuned cancellation between these large coefficients.
This cancellation is precise for smaller values of $N$ and gradually increases
with $N$.
...
carry out a numerical quadrature for mulitiple limb darkening models using
the approach of \citet{Pal2012}.
Their approach does not yet include the computation of derivatives, but
it does allows for a wider range of limb darkening models than polynomial, it
