...
Before we proceed we need to introduce some additional notation.  The flux of the star, 
$F$, we express as a linear combination of basis functions, $\mathfrak{s}_i(r,b)$, 
where $0 \le i \le N$, where $N$ is the order of the limb-darkening.  The total flux 
is given by the inner product of a vector of coefficients, $\mathfrak{g}$, with 
the set of basis vectors, where $\mathfrak{g}$ depends upon only upon the limb-darkening 
coefficients, but is independent of time.  For example, in the quadratic limb-darkening case, 
...
\rodrigo{Rodrigo: I took a stab at this in the paragraph above.  I don't yet make
reference to starry, though.}
...
$\mathrm{cel}$ with respect to the input parameters are given in Appendix \ref{app:cel_derivative}.
