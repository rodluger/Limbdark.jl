...
In terms of $b$, the normalized radius projected onto 
...
introduce some more notation.  For the quadratic limb-darkening case, the
flux is defined as
\begin{eqnarray}
...
\rodrigo{Yeah, I'm not sure what is going on, but I think that Goldberg published
the idea based Kahan's lectures, and then Kahan wrote up a summary many years
later.}
...
${\rm cel}$ with respect to the model parameters are given in Appendix \ref{app:cel_derivative}.
...
\todo{Eric: The following equation is breaking the compile on Travis.}
...
\lambda &=& k_c^2(b+ap-2bp)+p(3bp-ap^2-2b),\\
\frac{\partial {\rm cel}(k_c,p,a,b)}{\partial a } &=& {\rm cel}(k_c,p,1,0),\\
\frac{\partial {\rm cel}(k_c,p,a,b)}{\partial b} &=&{\rm cel}(k_c,p,0,1).
\end{eqnarray}
