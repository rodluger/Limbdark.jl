...
{\color{red} Include the quadratic limb-darkening expressions.}
...
to obtain a more stable expression for $\mathcal{P}_{\bvec{G}_n}$:
\begin{equation}
...
the first four terms analytically.
...
&=& \sum_{j=0}^{j_{max}} \mathcal{b}_j k^{2j},\\
\mathcal{b}_0 = \sqrt{\pi} \frac{\Gamma(\tfrac{n}{2}+1)}{\Gamma(\tfrac{n}{2}+\tfrac{3}{2}}(1-(r-b)^2)^{n} k,\\
\mathcal{b}_j = \mathcal{b}_{j-1} \frac{(2j-1)^2}{2j(1+n+2j)},\\
\end{eqnarray}
while for $k^2 > 1$,
\begin{eqnarray}
\mathcal{M}_n &=& \pi k^n _2F_1(\tfrac{1}{2},-\tfrac{n}{2};1;k^{-2}),\\
&=& \sum_{j=0}^{j_{max}} \mathcal{c}_j k^{-2j},\\
\mathcal{c}_0 = \pi (1-(r-b)^2)^{n},\\
\mathcal{c}_j = \mathcal{c}_{j-1} \frac{(2+n+2j)(1-2j)}{4j^2},\\
\end{eqnarray}


{\color{red} Give expressions for lowest four terms in $k^2 < 1$ and $k^2 >1$ limits,
as well as the series expansions.}
...
\frac{\partial \mathcal{P}}{\partial b} &=& \frac{n}{b} \left[(r^2+b^2)(\mathcal{M}_n - \mathcal{M}_{n-2})+(r^2-b^2)\mathcal{M}_{n-2}\right].\\
\end{eqnarray}
