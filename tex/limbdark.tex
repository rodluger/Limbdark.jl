...
We can express this in terms of a sequence of integrals,
...
\begin{eqnarray}
\mathcal{M}_1 &=& 2 (4br)^{1/2} k^2 cel(k_c,1,1,0),\\
\mathcal{M}_3 &=& \tfrac{2}{3}(4br)^{3/2} \left[k^2 cel(k_c,1,1,kc^2)+(3k^2-2)k^2 cel(k_c,1,1,0)\right],\\
\end{eqnarray}
for $k^2 \le 1$, while for $k^2 > 1$,
\begin{eqnarray}
\mathcal{M}_1 &=& 2(1-(r-b)^2)^{1/2} cel(k_c,1,1,k_c^2),\\
\mathcal{M}_3 &=& \tfrac{2}{3}(1-(b-r)^2)^{3/2} \left[(3-2k^{-2}) cel(k_c,1,1,k_c^2)+k^{-2} cel(k_c,1,1,0\right].
\end{eqnarray}
...
&=& \sum_{j=0}^{j_{max}} \alpha_j k^{2j},\\
\alpha_0 &=& \sqrt{\pi} \frac{\Gamma(1+\tfrac{n}{2})}{\Gamma(\tfrac{3}{2}+\tfrac{n}{2})}(1-(r-b)^2)^{n/2} k,\\
\alpha_j &=& \alpha_{j-1} \frac{(2j-1)^2}{2j(1+n+2j)},\\
\end{eqnarray}
while for $k^2 > 1$,
\begin{eqnarray}
\mathcal{M}_n &=& \pi (4br)^{n/2}k^n \,_2F_1(\tfrac{1}{2},-\tfrac{n}{2};1;k^{-2}),\\
&=& \sum_{j=0}^{j_{max}} \beta_j k^{-2j},\\
\beta_0 &=& \pi (1-(r-b)^2)^{n/2},\\
\beta_j &=& \beta_{j-1} \frac{(2+n+2j)(1-2j)}{4j^2},\\
\end{eqnarray}
where $j_{max}=\infty$ for $k^2 \le 1$ and for $k^2 > 1$ with odd $n$.  For
$k^2 > 1$ and even $n$, $j_{max} = n/2$;  in practice we set $j_{max} = 100$.  
The series is truncated when a term goes below a tolerance specified
by the numerical precision.

{\color{red} Give the Taylor series expansions near $b=0$.}
...
As with \starry, we find that upward recursion in $n$ is more stable for $\tfrac{1}{2} < k^2 < 2$,
while downward recursion is more stable for $k^2 < \tfrac{1}{2}$ and $k^2 > 2$. 
...
need to be carried out once.  We define a structure to hold the variables
which are reused throughout the light curve, and we also use this structure
to store variables which are used throughout a given computation to save
on memory allocation and on computation time.

Some terms only need to be computed once.  Once the number of limb-darkening
terms, $n$, is specified, then the series coefficients for $\mathcal{M}_n$,
$\alpha_j$ and $\beta_j$, are a simple function of $j$ and $n$, and so we 
compute these coefficients once, and store them in a vector for $k^2 \le 1$ 
and $k^2 > 1$, separately for $n_{max}-3$ to $n_{max}$, where $n_{max}=n+1$.
...
repeated terms are only computed once;  this gives efficiency
...
Other quantities, such as the denominator of the flux, $F_0$, need to
only be computed once per light curve.  Due to the greater computational
expense of square-roots and divisions, where possible we try to only
compute a square root or division once, storing these in a variable, and
then reusing these with cheaper multiplication throughout the computation
when needed.  For instance, for many formulae we require the
inverse of an integer, so an array of integer inverses is computed once
and stored, and then accessed as required rather than recomputed.
