...
\todo{Finish explaining the paper layout.}

    % \S\ref{sec:time}
    % \S\ref{sec:???} implementation
    % \S\ref{sec:???} benchmarking
    % \S\ref{sec:nonlinear}
    ...
    % Appendix: errata, derivs of cel and a table
...
%
\begin{proof}{A}
    \mathcal{A} = \left(\begin{matrix}-1 & -1 & - \frac{3}{2} & - \frac{5}{2} & - \frac{13}{3} & - \frac{23}{3}\\0 & 1 & 2 & \frac{18}{5} & \frac{32}{5} & \frac{80}{7}\\0 & 0 & - \frac{1}{4} & - \frac{3}{4} & - \frac{5}{3} & - \frac{10}{3}\\0 & 0 & 0 & \frac{1}{5} & \frac{4}{5} & \frac{15}{7}\\0 & 0 & 0 & 0 & - \frac{1}{6} & - \frac{5}{6}\\0 & 0 & 0 & 0 & 0 & \frac{1}{7}\end{matrix}\right)
\end{proof}
%
...
to unity, but is some unknown value.  So, the correct procedure is to 
multiply $F(t)$ by a constant which represents the unocculted flux.  
...
must be multiplied by the same constant.
...
on a 2.3 GHz
% SEE https://travis-ci.org/rodluger/limbdark/jobs/527913189#L545
...
\todo{Eric: This section seems a bit out of place, sandwiched in between
benchmarking and comparison to other codes. Perhaps it should go right
after the time integration section, before implementation details? Or
perhaps in the application section, followed by a blurb on how we plan
to derive a better, more flexible analytic limb-darkening model in
upcoming work? -RL}
...
\todo{Eric: I'm confused by the sentences above. It's partly the passive tense
in the first sentence that's throwing me off: you are referring to our current
paper, correct? But in the second sentence, you say ``it also uses a series
approximation...'' but I think you are now referring to Exofast? -RL}
...
and Cooperative Agreement Number NNA13AA93A.  This research was partially conducted during the Exostar19 program at the Kavli Institute for Theoretical Physics at UC Santa Barbara, which was supported in part by the National Science Foundation under Grant No.\ NSF PHY-1748958.
