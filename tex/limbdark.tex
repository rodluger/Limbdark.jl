...
analytic solution;  however, we find that the standard formula leads to
round-off error which is larger than desirable.  In this section we present
a new formula which we find yields machine precision for the area of
overlap, and its derivatives.

Figure \ref{fig:circle_overlap} shows how the area of overlap can be computed
for two circles.  The sums of the areas of the sectors which overlap minus
the area of a kite which spans them gives the lens-shaped area of overlap of
the two circles.
...
\begin{dcases}
0 & \qquad 1+r \le b,\\
\left[r^2 \kappa_0 + \kappa_1  \right.
                &\\ \phantom{XXXX}
...
However, we find that numerical round-off error limits the precision of this formula when
...
\begin{eqnarray}
A_{kite} &=& \frac{1}{4}\sqrt{(a+(b+c))(c-(a-b))(c+(a-b))(a+(b-c))},\\
a \ge b \ge c,
\end{eqnarray}
where the tuple ${a,b,c}$ equals ${1,r,b}$ sorted from from
greatest to least.

Next, the inverse cosine formulaes are also imprecise when $\cos{\kappa_0} = x_0 \approx
1$ and  $\cos{\kappa_1} = x_1 \approx 1$.  The approximate solution
...
Instead, we use the $\theta = atan2(y,x)$ function with $y=\sin{\theta}$ and
$x=\cos{\theta}$, which avoids the quadrant and
division-by-zero problems of the $\theta = tan^{-1}(y/x)$ function.  In particular,
\begin{eqnarray}
...
in the arctangent formula these denoninator terms cancel, which helps with 
numerical stability for small values of $b$.  

This gives the equations:
\begin{eqnarray} \label{eqn:area_of_overlap}
A_{lens} &=& \kappa_1 + r^2\kappa_0 - A_{kite},\\
\kappa_0 &=& atan2(2A_{kite},(r-1)(r+1)+b^2),\\
\kappa_1 &=& atan2(2A_{kite},(r-1)(r+1)-b^2).
\end{eqnarray}

The performance of this formula relative to the standard formula is profiled
in Figure \ref{fig:area_of_overlap_precision} for $r=0.1$, a typical
...
subtracted the results for comparison.  We find that the standard
formulae (equation \ref{eqn:MAuniform}) approaches errors of $10^{-8}$
in the limits$b \rightarrow 1-r$, with an error that exceeds the value 
of the area of the smaller circle minus the area of overlap for
values of $1-r < b < 1-r+10^{-8}$.  Thus, even though this computation
...
that matches or exceeds double-precision in both limits.  Figure
\ref{fig:precision_overlap} shows that equation \ref{eqn:area_of_overlap} 
gives a precision of $\approx 10^{-17}$ in the limit $b \rightarrow
1-r$ for $r=0.1$, while even higher precision is achieved
for $b \rightarrow 1+r$.

The final modification we make is that when computing the uniform light
curve, we require $\pi-A_{lens}$, so rather than computing $\kappa_1$
we compute $\pi-\kappa_1 = -atan2(\sin{\kappa_1},\cos{\kappa_1})$.
In the remainder of this paper we will use these formulae in computing
the higher order limb-darkening coefficients.

\begin{figure}\label{fig:circle_overlap}
    \begin{centering}
    \includegraphics[width=0.85\linewidth]{Circle_overlap_area.pdf}
...
\begin{figure}\label{fig:overlap_precision}
    \begin{centering}
    \includegraphics[width=0.85\linewidth]{figures/area_of_overlap.pdf}
    ...
    points of contact) and $b=1+r$ (first and fourth points of contact).}
    \end{centering}
\end{figure}
