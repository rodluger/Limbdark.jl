...
the coefficients $y_l$ for which we may write
%
\begin{align}
    \label{eq:sphharmld}
    \frac{I(\upmu)}{I(1)} &= \sum_{l=0}^{l_\mathrm{lmax}} y_l Y_{l,0}(\upmu)
    \quad.
\end{align}
%
...
\begin{align}
    ...
    \frac{I(\upmu)}{I(1)} &= \bvec{y}^\top \, \bvec{M} \, \pmb{\upmu}
    \quad,
\end{align}
%
where $\bvec{y}^\top$ is a row vector whose value at index
$l$ is $y_l$ and $\bvec{M}$ is the lower triangular matrix with components given by
%
\begin{align}
    \label{eq:Mlk}
...
%
\begin{align}
    \bvec{y}^\top \, \bvec{M} = \bvec{u}^\top \, \bvec{L} \quad,
\end{align}
%
or
%
\begin{align}
    \bvec{y} = (\bvec{M}^{-1} \, \bvec{L})^\top \bvec{u} \quad.
\end{align}


\begin{figure}[t!]
    \begin{centering}
    \includegraphics[width=0.5\linewidth]{figures/python/python.pdf}
