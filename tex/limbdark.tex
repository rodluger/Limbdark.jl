...
%\label{sec:poly}
% For convenience, this can be expressed as the matrix equation
% %
% \begin{align}
...
%     \caption{\label{fig:high_order_ld} Tenth order limb-darkening example with \starry. \pycodelink{high_order_ld}}
%     \end{centering}
% \end{figure}
...
\end{align}
%
where $I_0 \equiv I(\upmu=1)$ and we defined $u_0 \equiv -1$.
%
...
$\mubasis \equiv (1 \ \upmu \ \upmu^2 \ ... \ \upmu^N)^\mathsf{T}$, we may 
express Equation~(\ref{eq:polynomialld}) more compactly as the
dot product
%
\begin{align}
    \label{eq:polynomialld_vec}
    \frac{I(\upmu)}{I_0} &= \mubasis^\mathsf{T} \bvec{u} .
\end{align}

Our task is to compute the flux $\mathcal{F}$ observed during an occultation by
...
\begin{align}
    ...
    \nonumber \\
    &= 
    I_0 \oiint \mubasis^\mathsf{T} \, \dd S \, \bvec{u} \quad.
\end{align}
%
...
\begin{align}
    ...
    \mathcal{F} &= I_0 \oiint \gbasis^\mathsf{T} (\upmu) \, \dd S \, \bvec{A} \bvec{u} \nonumber \\
...
\begin{align}
    ...
    \mathfrak{s}^\mathsf{T} \equiv \oiint \gbasis^\mathsf{T} (\upmu) \, \dd S
\end{align}
%
...
where $\gbasis$ is some complete basis of polynomials in $\upmu$.
...
the occulted body's disk:
%
%
\begin{align}
    \label{eq:greens}
    \mathfrak{s} &=
    \oint \bvec{G} (\upmu) \cdot \dd \bvec{r}
    \quad,
\end{align}
%
where $\bvec{G}$ is a vector whose $n^{\mathrm{th}}$ component is the
vector field
%
\begin{align}
    \label{eq:greens_n}
    \bvec{G}_n (\upmu) = {G_n}_x (\upmu) \, \xhat + {G_n}_y (\upmu) \, \yhat \quad.
\end{align}
%
The components ${G_n}_x$ and ${G_n}_y$ are chosen such that
%
\begin{align}
    \label{eq:DGg}
...
\end{align}
%
As in \citet{Pal2008} and \citet{starry}, the operation 
...
limb-darkening.  Following \citet{starry}, we choose a Green's function
of the form
\begin{equation}
...
along with the uniform, $\tilde{\mathfrak{g}}_0 = z^0$,
and linear, $\tilde{\mathfrak{g}}_1 = z^1$, terms which we have derived in \S~\ref{sec:uniform} and 
\S~\ref{sec:reparam}.
Note that the total flux of each term in this basis set, $\tilde{\mathfrak{g}}_n$, integrates
...
in section \ref{eq:uniform}, $\mathfrak{s}_1$ is given by the linear limb-darkened
...
The integrals $\mathcal{M}_n$ obey a straightforward recursion relations
