...
\todo{I think the expression for $\dd\Lambda/\dd b$ for $b = r < 1/2$ is slightly wrong.}
...
\begin{align}
    ...
    \frac{I(\upmu)}{I(1)} &= 1 - u_1 (1 - \upmu) - u_2 (1 - \upmu)^2 - ... - u_{N}(1 - \upmu)^{N} \nonumber \\
                          &= 1 - \sum_{i=1}^N u_i \sum_{j=0}^i \binom{i}{j} (-1)^j \upmu^j .
\end{align}
...
\todo{This can be written as the dot product of the mu basis with the u coefficients,
or as the dot product of the g basis, a change of basis matrix, and the u coefficients.
This makes integrating easier. Show some math here.}
...
\begin{align}
    ...
    \nonumber \\
    &= 
    \oiint \gbasis^\mathsf{T} (\upmu) \, \dd S \, A (\bvec{u}) \quad.
\end{align}
