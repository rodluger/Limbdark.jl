...
unstable for small values of $b$ due to the small difference between the two
terms which are subtracted, followed by division by $b$.  To avoid this problem
...
which are given by:
\begin{eqnarray}
\mathcal{N}_0 &=& \frac{\kappa_0}{2} - k k_c,\\
...
In the $k^2 < 1/2$ limit, we find the recursion to be unstable, and so we evaluate
...
\end{eqnarray}
For $k^2 < 1/2$, we compute the values of $N_{n}$ and $N_{n-1}$, and use downward
