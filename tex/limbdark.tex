...
\begin{align}
    ...
    \end{dcases}
\end{align}
Note that $\kappa_0$, $\kappa_1$, and $A_{kite}$ were already computed in the
uniform case, so these quantities are reused in the quadratic case.
...
In practice we substitute $\mathcal{M}_{n+2}$ with the recursion relation
to obtain a more stable expression for $\mathcal{P}(\bvec{G}_n)$:
\begin{equation}
...
\end{eqnarray}
Note that we express the elliptic integrals in terms of the $cel$ integrals which
...
When $k^2 < 1$, the integrals may be expressed in terms of Hypergeometric functions
and an infinite series:
\begin{eqnarray}
\mathcal{M}_n &=& (4br)^n k^{2n+1} \pi^{1/2} \frac{\Gamma(n+1)}{\Gamma(n+\tfrac{3}{2})} _2F_1(\tfrac{1}{2},\tfrac{1}{2};\tfrac{3}{2}+n;k^2),\\
&=& \sum_{j=0}^{j_{max}} b_j k^{2j},\\
b_0 &=& \sqrt{\pi} \frac{\Gamma(\tfrac{n}{2}+1)}{\Gamma(\tfrac{n}{2}+\tfrac{3}{2}}(1-(r-b)^2)^{n} k,\\
b_j &=& b_{j-1} \frac{(2j-1)^2}{2j(1+n+2j)},\\
\end{eqnarray}
while for $k^2 > 1$,
\begin{eqnarray}
\mathcal{M}_n &=& \pi k^n _2F_1(\tfrac{1}{2},-\tfrac{n}{2};1;k^{-2}),\\
&=& \sum_{j=0}^{j_{max}} c_j k^{-2j},\\
c_0 &=& \pi (1-(r-b)^2)^{n},\\
c_j &=& c_{j-1} \frac{(2+n+2j)(1-2j)}{4j^2},\\
\end{eqnarray}
where $j_{max}=\infty$ for $k^2 < 1$ and for $k^2 > 1$ with odd $n$.  For
