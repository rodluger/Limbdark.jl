...
# Now, compute the d_n coefficients:
for j=t.n:-1:2
  if j >= t.n-1
...
function compute_d_n_grad!(t::Transit_Struct{T}) where {T <: Real}
# Transform the u_n coefficients to d_n, which are coefficients
# of the basis in which the P(G_n) functions are computed.
# Compute the derivatives of the flux with respect to the u coefficients.
# We define d_n with two extra elements which are zero:
a_n = zeros(T,t.n+1)
...
# Now, compute the d_n coefficients and propagate derivatives:
for j=t.n:-1:2
  if j >= t.n-1
    t.d_n[j+1] = a_n[j+1]/(j+2)
    for i=1:t.n
