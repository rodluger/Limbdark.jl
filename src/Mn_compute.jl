...
#    M_n/(4*b*r)^{m/2} = \int_{-\kappa/2}^{\kappa/2} (k^2-\sin^2{x})^{m/2} dx
...
# Now, compute higher order terms until desired precision is reached:
  @inbounds for j=1:4
    ...
    t.Mn[m_max-3+j] = t.Mn_coeff[1,j,1]
    k2n = one(T)   # k^{2n}
  ...
  @inbounds for j=1:4
    t.Mn[m_max-3+j] = t.Mn_coeff[2,j,1]
    k2n = one(T)
    @inbounds for n = 1:t.jmax-1
      k2n *= t.k2inv
...
# Recursive computation of M_n starting at m=1 to 4, and raising to m_max:
...
if k2 < 1.0
  ...
  t.Mn[4] = (2*t.sqbr)^3*t.twothird*(k2*t.Eofk+k2*(3*k2-2)*t.Em1mKdm)
else
  t.Mn[1] = pi
...
# Recursive computation of M_n starting at m=m_max to m_max-3, and lowering to m=1:
