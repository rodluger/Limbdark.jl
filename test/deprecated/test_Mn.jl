...
# Initialize all the variables needed for computing Mn:
...
# Carries out a test of M_n for k^2 and m_max:
function test_Mn(k2::Float64)
@assert (k2 > 0)
m_max = 30
...
t.k2 = k2
t_big.k2 = big(k2)
# Initialize variables needed for computing M_n:
initialize_big(t)
initialize(t_big)
...
for m=0:t.m_max
  Mnn = Mn_num(k2,m)
  @test isapprox(Mnn,t.Mn[m+1],atol=abstol,rtol = reltol)
  if !isapprox(Mnn,t.Mn[m+1],atol=abstol,rtol = reltol)
    println("m: ",m," Mn_num: ",Mnn," M_n: ",t.Mn[m+1]," diff: ",Mnn-t.Mn[m+1])
  end
  @test isapprox(t.Mn[m+1],convert(Float64,t_big.Mn[m+1]),atol=abstol,rtol=reltol)
  if !isapprox(t.Mn[m+1],convert(Float64,t_big.Mn[m+1]),atol=abstol,rtol=reltol)
    println("m: ",m," Mn: ",t.Mn[m+1]," M_n_big: ",convert(Float64,t_big.Mn[m+1])," diff: ",t.Mn[m+1]-convert(Float64,t_big.Mn[m+1]))
  end
end
return
end
...
for i=1:ntest
  test_Mn(.5*rand(seed))
  test_Mn(.5+.5*rand(seed))
  test_Mn(inv(.5+.5*rand(seed)))
  test_Mn(inv(.5*rand(seed)))
end
end
